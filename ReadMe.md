...
The database is organized around the major entities involved in an e-commerce transaction lifecycle.
...
```text
                    ...
                    ▼                         ▼
          ┌──────────────────┐       ┌─────────────────┐
          │CUSTOMER_ADDRESSES│       │ SHOPPING_CART   │
          └────────┬─────────┘       └────────┬────────┘
                   │                          │
                   │                          ▼
                   │                 ┌─────────────────┐
...
```text
Database-of-End-to-End-E-Commerce-Platform/
│
├── DDL/
│   └── ddl.sql
│
...
This creates the core tables, constraints and relationships.

## 2. Compile PL/SQL Packages

Compile the packages in dependency order:

```text
PACKAGES/PKG_CATALOG.sql
PACKAGES/PKG_SECURITY.sql
PACKAGES/PKG_INVENTORY.sql
PACKAGES/PKG_CART.sql
PACKAGES/PKG_ORDERS.sql
PACKAGES/PKG_SHIPPING.sql
```
...
```text
TRIGGERS/TRG_AUDIT_ORDERS.sql
TRIGGERS/TRG_STOCK_ALERT.sql
```
...
The schema is designed to minimize redundancy and reduce update, insertion and deletion anomalies.

### ✔ Strong Referential Integrity

Foreign-key relationships maintain consistency between related entities.

### ✔ Constraint-Based Validation

Invalid data and business states are rejected at the database level wherever appropriate.

### ✔ Transaction-Safe Order Processing

Checkout is implemented as a coordinated transactional workflow instead of a collection of unrelated SQL statements.

### ✔ Concurrency Control

Inventory rows are locked during critical operations using `SELECT ... FOR UPDATE`.

### ✔ Modular PL/SQL

Business logic is separated into domain-specific packages.

### ✔ Automated Database Operations

Triggers automatically generate audit information and low-stock alerts.

### ✔ Auditability

Important order changes can be recorded with old/new values, user information and timestamps.
